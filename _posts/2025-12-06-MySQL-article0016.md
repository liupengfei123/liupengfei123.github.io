...
## 使用双写缓冲区保障页面原子化刷入磁盘
InnoDB为了保证16KB的页面一致性，写入磁盘需是原子的，即要么16KB页面完全写入，要么完全不写入。但是，传统的磁盘操作通常不能保证16KB的原子写入（因为磁盘的扇区通常为512字节或4KB）。

所以InnoDB通过双写缓冲区(系统表空间特殊区域，连续的128个页总共2MB) 来保证磁盘中页面的一致性
1. 当需要将flush脏页落盘时，会先将这些脏页写入到双写缓冲区中（内存）。
2. 随后在将双写缓冲区中的页面落盘到系统表空间的文件中。（因为是连续页面所以比较快）
3. 再双写缓冲区落盘成功之后再将flush脏页落盘到实际文件中。
4. 所以当flush脏页时落盘崩溃，出现页面损坏就可以通过双写缓冲区中以及保存的副本进行还原。

总结：**InnoDB通过双写缓冲区机制来保证16KB页面的原子写入。它并不是依赖磁盘的原子写能力，而是通过冗余副本来实现崩溃恢复**。

在恢复时，InnoDB会先检查数据页的完整性（通过双写缓冲区），然后使用重做日志来重做已提交的事务。

PS：
1. 页面是否损坏通过页面的校验和
2. 当现代的一些存储设备（如SSD）可能支持原子16KB写入），可以禁用双写缓冲区以提高性能。可以通过设置innodb_doublewrite=0来禁用双写缓冲区。
